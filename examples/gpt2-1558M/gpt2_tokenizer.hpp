//
// credit: https://github.com/gf712/gpt2-cpp
//
#include <string>
#include <fstream>
#include <memory>
...
static inline size_t codepoint_length( const char c )
{
...
static inline std::unordered_map<char, std::string> bytes_to_unicode()
{
...
static inline std::unordered_map<std::string, char> unicode_to_bytes()
{
...
struct gpt2_tokenizer
{
    ...
    static constexpr ctll::fixed_string pattern {"'s|'t|'re|'ve|'m|'ll|'d| ?\\p{L}+| ?\\p{N}+| ?[^\\s\\p{L}\\p{N}]+|\\s+(?!\\S)|\\s+"};
    ...
    static std::optional<gpt2_tokenizer> load( std::string const& vocab_file, std::string const& merges_file )
    {
        spdlog::info( "Loading gpt2 tokenizer from vocab file {} and merges file {}", vocab_file, merges_file );
        // load merges file
        ...
        {
            spdlog::error( "Failed to open merges file from {}", merges_file  );
            return std::nullopt;
        }
        ...
        if ( error )
        {
            spdlog::error( "Failed to read json from from vocab file {}", vocab_file );
            return std::nullopt;
        }
        ...
        spdlog::info( "byte_encoder has {} records.", result.m_byte_encoder.size() );
        result.m_byte_decoder = unicode_to_bytes();
        spdlog::info( "byte_decoder has {} records.", result.m_byte_decoder.size() );
        return result;
    }
    ...
    std::vector<std::string> bpe( const std::string& token )
    {
        ...
        while ( true )
        {
            ...
            (
                ...
                {
                    if ( lhs == m_bpe_ranks.end() && lhs == m_bpe_ranks.end() ) { return false; }
                    if ( lhs == m_bpe_ranks.end() || rhs == m_bpe_ranks.end() ) { return ( lhs != m_bpe_ranks.end() ); }
...
inline std::optional<gpt2_tokenizer> make_gpt2_tokenizer( std::string const& vocab_file, std::string const& merges_file )
{
    return gpt2_tokenizer::load( vocab_file, merges_file );
}
